"""
Comando corregido para cargar datos reales desde archivos CSV
"""
import csv
import os
...
class Command(BaseCommand):
    help = 'Carga datos reales desde archivos CSV (versión corregida)'
            ...
            for row in reader:
                if row['ACTIVX'] == 'SI':  # Solo docentes activos
                    try:
                        ...
                        # Limpiar número de registro
                        nro_registro = row['Nº de Registro'].replace('.', '').replace(',', '').strip()
                        if not nro_registro or nro_registro == '':
                            continue
                        
                        # Crear docente
                        docente, created = Docente.objects.get_or_create(
                            legajo=int(nro_registro),
                            defaults={
                                'nombre': nombre_completo,
                                'apellido': apellido,
                                'dni': int(row['DOCUMENTO'].replace('.', '').replace(',', '')),
    ...
    def setup_materias_csv(self):
        """Crear materias con docentes desde archivo CSV (sin encabezados)"""
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            for i, line in enumerate(file, 1):
                try:
                    ...
                    # Campos: ID, Año, Curso, Materia, Docente, Email, ...
                    curso_nombre = campos[2].strip()  # Curso
                    materia_nombre = campos[3].strip()  # Materia
                    docente_nombre = campos[4].strip()  # Docente
                    
                    if not curso_nombre or not materia_nombre or not docente_nombre:
                        continue
                    ...
                    if docente:
                        ...
                        # Log cada 100 materias
                        if i % 100 == 0:
                            self.stdout.write(f'    📚 {i}/{total_rows} materias procesadas...')
                    
                except Exception as e:
                    if i % 100 == 0:  # Solo mostrar errores cada 100
                        self.stdout.write(f'    ⚠️ Error en línea {i}: {e}')
            ...
            for i, row in enumerate(reader, 1):
                if row['ACTIVX'] == 'SI':  # Solo estudiantes activos
                    try:
                        ...
                        # Limpiar DNI
                        dni_str = row['NUMERO'].replace('.', '').replace(',', '').strip()
                        if not dni_str:
                            continue
                        ...
                        if created:
                            ...
                            if row.get('NOMBRE MADRE') and row['NOMBRE MADRE'].strip():
                                familiar = Madre.objects.create(
                                    ...
                                    telefono_madre=row.get('Tel Madre', '') or f'11-{random.randint(1000, 9999)}-{random.randint(1000, 9999)}',
                                    nacionalidad_madre=row.get('NACIONALIDAD P', 'Argentina'),
                                    direccion_madre=row['DOMICILIO'] or f'Calle {random.randint(100, 9999)} #{random.randint(1, 999)}'
                                )
                                alumno.madre = familiar
                                alumno.save()
                            elif row.get('NOMBRE PADRE') and row['NOMBRE PADRE'].strip():
                                familiar = Padre.objects.create(
                                    ...
                                    telefono_padre=row.get('Tel Padre', '') or f'11-{random.randint(1000, 9999)}-{random.randint(1000, 9999)}',
                                    nacionalidad_padre=row.get('NACIONALIDAD P', 'Argentina'),
                            ...
                            User.objects.create_user(
                                ...
                                email=row.get('EMAIL', '') or f"{row['NOMBRES'].lower().replace(' ', '.')}.{row['APELLIDO'].lower()}@alcal.edu.ar"
                            )
                        ...
                        if alumnos_creados % 50 == 0:  # Solo mostrar errores cada 50
